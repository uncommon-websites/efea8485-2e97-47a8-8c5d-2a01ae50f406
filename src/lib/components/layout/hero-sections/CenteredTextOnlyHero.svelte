...
<div class="grid h-[calc(100vh-var(--nav-height))] grid-rows-[1fr_auto]" {...rest}>
	<div class="bg-background relative isolate flex items-center justify-center">
		...
		<header
			class={[
				"section-px z-50 container mx-auto grid place-items-center text-center text-balance",
				imageSrc ? "dark:text-white" : ""
			]}
			data-enter-container
		>
			<div class="grid max-w-prose place-items-center justify-center gap-6 lg:max-w-[80ch]">
				<h1 class={[
					"text-display w-full text-balance lg:!text-7xl",
					imageSrc ? "dark:text-shadow-lg" : ""
				]} data-enter>
				...
				<p
					data-enter
					class={[
						"text-headline block max-w-[45ch] text-pretty opacity-80 transition duration-500 ease-out",
						imageSrc ? "dark:text-shadow-lg" : ""
					]}
				>
					{subtitle}
